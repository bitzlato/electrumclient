...
class ElectrumBatchClient(ElectrumClient):
    class Req:
        ...
        def __init__(self, req_id: str, method: str, params: List,
                     result: Any = None, errors: Any = None, resp_validate_fun: Callable = None):
            self.req_id = req_id
            self.method = method
            self.params = params
            self.result = result
            self.errors = errors
            self.resp_validate_fun = resp_validate_fun

        def set_result(self, res):
            if self.resp_validate_fun is not None and self.resp_validate_fun(res) is False:
                raise Exception("Validation ERROR")
            self.result = res
            return self

    def __init__(self, network: Network,  loop, stopping_fut, loop_thread, *,
                 batch_limit: int = 50, raise_error: bool = False):
    ...
    def get_balances(self, script_hash: str, *args, **kwargs) -> str:
        return self.add_request("blockchain.scripthash.get_balance", [script_hash], resp_validate_fun=None)

    def get_listunspents(self, script_hash: str, *args, **kwargs) -> str:
        return self.add_request("blockchain.scripthash.listunspent", [script_hash], resp_validate_fun=None)

    def get_listmempools(self, script_hash: str, *args, **kwargs) -> str:
        return self.add_request("blockchain.scripthash.get_mempool", [script_hash], resp_validate_fun=None)

    def add_request(self, method: str, params: List, resp_validate_fun: Callable = None, *args, **kwargs):
        req_id = str(uuid.uuid4())
        self.requests.update({
            req_id: self.Req(req_id=req_id, method=method, params=params, resp_validate_fun=resp_validate_fun)
        })
    ...
    async def __run__(self, requests, **kwargs):
        ...
        for chunk in self.chunks(list(requests.items()), self.batch_limit):
            ...
            count += len(chunk)
            if count % 1000 == 0:
                self.logger.info(f"{thread_name} -- count={count}")
...
class ElectrumThreadBatchClient(ElectrumBatchClient):
    def __init__(self, network: Network, loop, stopping_fut, loop_thread, *,
        ...
        super().__init__(network, loop, stopping_fut, loop_thread, batch_limit=batch_limit, raise_error=raise_error)
